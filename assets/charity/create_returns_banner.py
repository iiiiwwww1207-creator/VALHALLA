...
def add_cards(base: Image.Image) -> None:
    ...
    upper = (
        ("LIVE", "ライブプラン", "¥10,000", "限定30口 ／ スタンディング後方",
         ("アコースティックライブ観覧",)),
        ("FRONT ROW", "VIPプラン", "¥50,000", "限定15口 ／ スタンディング最前列 ／ 18:00 先行入場",
         ("最前列エリアで観覧", "MIOとのチェキ＋ハイタッチ（18:15〜）",
          "限定名刺（サイン入り）", "［11/28］MIOが梵字を書きます",
          "［11/28］シャンパンコール観覧・うちわ・ゲーム")),
        ("VVIP", "VVIPプラン", "¥330,000", "限定5口 ／ 同伴1名まで可（特典は2名分）／ ソファー席",
         ("MIOがお席にご挨拶に伺い6分", "チェキ／写真／動画OK", "限定名刺（サイン入り）",
...
def add_footer(base: Image.Image) -> None:
    ...
    text = (
        ...
        "ライブ終演 20:05　※収益から必要経費を除いた全額を教育文化セキュリティ財団へお渡しします"
    )
