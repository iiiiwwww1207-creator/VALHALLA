...
def add_cards(base: Image.Image) -> None:
    ...
    upper = (
        ("LIVE", "ライブ席", "¥10,000", "100名 ／ スタンディング",
         ("アコースティックライブ観覧",)),
        ("FRONT ROW", "最前列席", "¥50,000", "30名 ／ スタンディング最前列",
         ("最前列でご覧いただけます", "アコースティックライブ観覧",
          "［後日］ファンミーティング")),
        ("S SEAT", "S席・MIOタイム", "¥150,000", "15口 ／ 同伴1名まで可 ／ 相席 ／ 17:45 先行入場",
         ("お席をご用意します", "チェキ（1分）", "限定名刺", "なりきりグッズ",
          "［後日］ファンミーティング", "［後日］やしろ語録動画")),
        ("VVIP", "VVIP席", "¥330,000", "10口 ／ 同伴1名まで可 ／ 相席（席は選べます）",
         ("MIOと対面で6分", "チェキ／写真／動画OK", "限定名刺", "なりきりグッズ",
          "ライブ観覧", "［後日］30分グループセッション")),
    )
    lower = (
...
def add_footer(base: Image.Image) -> None:
    ...
    text = (
        ...
        "ライブ終演 20:05　※収益は必要経費を除いた全額で支援します"
    )
