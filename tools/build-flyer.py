...
DROP_LINES = {503.0, 763.0, 998.0, 1001.0, 1021.0, 1045.0, 1107.0, 0.0}
DROP_SIZES = {12.85, 14.74, 22.67, 9.82, 7.18}
...
DRAW = [
    # キャッチコピー。2026-09-08 に「ビジュアル系文化を守り、継ぐ。」から変更。
    # 元は 17pt。人物の足元（device y≈530）と日付（同 502）の下に
    # 37pt の余白があるので、そこに収まる最大まで上げる。
    (296.6, 462.0, "文化 × エンタメ × AI", 27.0, (255, 255, 255), 0.05),
    ...
    (297.5, 275.0, "お申し込みは CAMPFIRE にて", 9.2, CREAM, 0.06),
    (297.5, 264.0, "事前にお気に入り登録をお願いいたします", 7.8, CREAM, 0.04),
    # お金の文言。ページ・CAMPFIRE と同じ言い回しに揃える。
    (CENTER, 93.4, "収益から必要経費を差し引いた", 11.05, WHITE, 0.05),
    (CENTER, 76.2, "全額を教育文化セキュリティ財団へお渡しします", 11.05, WHITE, 0.05),
    (CENTER, 58.2, "※必要経費には、会場費・出演料・機材費・決済手数料が含まれます。",
     6.6, NOTE, 0.0),
    (CENTER, 47.0, "お渡しする金額は開催後にご報告します。", 6.6, NOTE, 0.0),
]
